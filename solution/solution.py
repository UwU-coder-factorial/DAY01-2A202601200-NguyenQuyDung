...
def call_openai(
    ...
    return response_text, latency_second
# ---------------------------------------------------------------------------
...
def compare_models(prompt: str) -> dict:
    ...
    pricing_mini = PRICING_PER_1K_TOKENS.get(
        OPENAI_MINI_MODEL,
        PRICING_PER_1K_TOKENS["gpt-4o-mini"],
    )

    gpt4o_cost = (len(gpt4o_answer.split()) / 0.75) / 1000 * pricing["output"]
    mini_cost = (len(mini_answer.split()) / 0.75) / 1000 * pricing_mini["output"]
...
def count_tokens(text: str, model: str = OPENAI_MODEL) -> int:
    ...
    try:
        ...
        # Fallback khi offline hoặc model không có encoding trong tiktoken
...
def run_assistant(
    ...
    while True:
        ...
        cost_dict = estimate_cost(prompt_str, reply, model=OPENAI_MODEL)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for res in results:
        p = res.get("prompt", "")
        p_short = p[:37] + "..." if len(p) > 40 else p
        
        g_ans = res.get("gpt4o_answer", "")
        g_ans_short = g_ans[:37] + "..." if len(g_ans) > 40 else g_ans
        
        m_ans = res.get("mini_answer", "")
        m_ans_short = m_ans[:37] + "..." if len(m_ans) > 40 else m_ans
        
        g_time = f"{res.get('gpt4o_time', 0.0):.3f}s"
        m_time = f"{res.get('mini_time', 0.0):.3f}s"
        
        row = f"{p_short:<40} | {g_ans_short:<40} | {m_ans_short:<40} | {g_time:<15} | {m_time:<15}"
        lines.append(row)
    return "\n".join(lines)
